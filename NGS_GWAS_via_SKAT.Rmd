...
grep -E -v '#' data/MMP.vcf | grep -E 'SN=[A-Z0-9.]{4,}|CODING=' | grep -E -v 'SN=I;' | grep -E -v 'GRANT=0' | cut -f 1-8 > data/gene_variants.txt
...
All_SKAT_Data$results <-All_SKAT_Data$results[order(All_SKAT_Data$results$Q.value),]
All_SKAT_Data$results[1:20,]

#save SKAT results
write.table(x = All_SKAT_Data$results, file = "data/SKAT_all.results", row.names = FALSE, col.names = TRUE, quote = FALSE, append = FALSE)
```
...
# perform SKAT on all sets of variants
All_SKAT_Data_reduced  <- SKAT.SSD.All(SSD.info_reduced, Null_Model) 
...
# sort SKAT results by q-value
All_SKAT_Data_reduced$results <- All_SKAT_Data_reduced$results[order(All_SKAT_Data_reduced$results$Q.value),]
