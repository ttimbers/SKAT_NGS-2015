...
## New considerations when perforiming GWAS on sequecing data
...
We need to first compress all the `.vcf` files. We will do this in the Shell using `gzip`.
```{r engine='bash'}
# Bash Shell
gzip data/*.vcf
ls data/*.vcf* | head -5
...
# had to run Stephane's perl script on each strain individually to get format and individual genotype columns

# pre-process .vcf files (maybe write a for loop?)
bgzip -c VC10118.vcf > VC10118.vcf.gz
tabix -p vcf VC10118.vcf.gz
bgzip -c VC20138.vcf > VC20138.vcf.gz
tabix -p vcf VC20138.vcf.gz

#not sure if this works, but someone said it did
#file=$1 
#bgzip >$file
#tabix -f -p vcf $file

# merge them together
# vcf-merge data/*.vcf.gz | bgzip -c > data/MMP.vcf.gz does not work
vcf-merge VC10118.vcf.gz VC20138.vcf.gz | bgzip -c > test_merge.vcf.gz # works but you get "."" instead of "0/0"" so have to replace them...
ls data/MMP.vcf*
```
