...
disease outcome). The method compares the DNA sequences of two groups: individuals with the diesease phenotype or disease outcome (cases)
and individuals without the diesease phenotype or disease outcome (controls). Individuals DNA is read via SNP arrays or sequencing and
...
* potentially increased power due to rare variants potentially bein 
...
2. Choose perform association analysis via either gene sets or windows (*e.g.* SNP/variant set is defined to be those 
variants which fall within a chosen region size).
3. (optional) Create a SNP set ID (`.SSID`) file which indicates which variants belong to which set (*e.g.,* gene)
...
that if there was no variant info for an individual, when it merged the file it assumed that the data was missing. In
the case of this data, it is not, each individual .vcf files simply did not contain all variants for this whole dataset
because each file would be ~ 860,000 lines long filled with mostly "0/0". But we need those "0/0" and so we need to replace 
...
`plink` (**reference**) is one of the most widely used programs to perform traditional GWAS (*e.g.,* using SNP array data) 
...
We find that if we choose a 30% false-discovery rate cut-off that we find several hundred genes significantly associated with
...
Now we can see that only 4 genes are associated with the phenotype - a much more reasonable number. So
...
3. Thompson O, Edgley M, Strasbourger P, Flibotte S, Ewing B, Adair R, Au V, Chaudry I, Fernando L, Hutter H et al. 2013. The Million Mutation Project: A new approach to genetics in Caenorhabditis elegans. Genome Res doi:gr.157651.113 [pii] 10.1101/gr.157651.113.
4. Wu MC, Lee S, Cai T, Li Y, Boehnke M, Lin X. 2011. Rare-variant association testing for sequencing data with the sequence kernel association test. American journal of human genetics 89: 82-93.